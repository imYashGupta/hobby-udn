...
app.get('/product', (req, res) => {
    const options = {
      method: 'GET',
      url: 'https://corsproxy.org/?https://api.udaan.com/api/foodFmcgSearch/v1/listing/TLFSRZE6X0NP8FXQDJZC58VJ5LWQNM9',
      headers: {
        Authorization: `Bearer eyJraWQiOiI2TW53IiwidHlwIjoiSldUIiwiYWxnIjoiSFM1MTIifQ.eyJhdWQiOiJodHRwczovL2FwaS51ZGFhbi5jb20vIiwic3ViIjoiVVNSMExKVjRENFcwVERSQjRNTlpINjZISDlaTDAiLCJyIjpbInVzZXIiXSwibmJmIjoxNjk3NDAwNDg2LCJpc3MiOiJhdXRoLnVkYWFuLmNvbSIsImV4cCI6MTY5NzQwMTA4NiwibyI6Ik9SRzFCOEVIRkpLMkZDN0RGS0xMUjJFOENaTkYwIn0.8kTwjQTA5FviVcboBX5YD7iFb1IaJ9xi_zcZYdT1dUSR-jtArWdH19xHaAgHQ_sddFat0Q1Unksa5vBr8M5GaA`
      }
    };
    ...
    .then(function (response) {
      ...
      res.json(response.data); // Send the response from the Axios request as the response for the Express route
    })
    .catch(function (error) {
      console.error(error);
      res.status(500).json({ error: 'An error occurred' }); // Send an error response if the request fails
    });
